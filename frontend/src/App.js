...
function App() {
  ...
  const loadCategories = async () => {
    try {
      ...
      const unique = Array.from(new Set(valid.map((q) => q.category))).sort(
  ...
  const selectAnswer = (selected, timeLeft) => {
    ...
    const correctAnswer = Array.isArray(correctIdxs)
      ? correctIdxs
      : [correctIdxs];
    ...
    if (isCorrect) {
      ...
      const clampedTime = Math.max(
        0,
        Math.min(TIME_LIMIT_SECONDS, timeLeft ?? 0),
      );
  ...
  const gameDurationMs =
    gameStartedAt && gameEndedAt ? gameEndedAt - gameStartedAt : null;
  ...
  if (!isAuthenticated) {
    return (
      <div className="App">
          ...
          <div className="TriviaContainer">
            <div className="HeroCard">
              ...
              <div className="ButtonsGrid">
                <button className="TriviaButton gradient" onClick={showLogin}>
                  Sign In
                </button>
                <button className="TriviaButton" onClick={showRegister}>
                  Create Account
                </button>
              </div>
            </div>
        ...
        {currentView === "login" && (
          <Login onLogin={handleLogin} onSwitchToRegister={showRegister} />
        )}

        {currentView === "register" && (
          <Register onRegister={handleRegister} onSwitchToLogin={showLogin} />
        )}
      </div>
    );
  }
  ...
  if (showProfile) {
    return (
      <div className="App">
        <Profile user={user} onBack={handleBackFromProfile} />
      </div>
    );
  }
  ...
  if (showStats) {
    return (
      <div className="App">
        <PlayerStats user={user} onBack={handleBackFromStats} />
      </div>
    );
  }
  ...
  const handleEarlyFinish = () => {
    if (
      window.confirm(
        "Are you sure you want to end the game now? Unanswered questions will be counted as incorrect and your progress will be lost.",
      )
    ) {
      setGameFinished(true);
      setGameEndedAt(Date.now());
      ...
      setWrongCount(
        (w) => w + (questions.length - (correctCount + wrongCount)),
      );
    }
  };
